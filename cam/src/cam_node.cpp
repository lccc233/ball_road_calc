...
float exp_tim=100;
int x_px=1280,y_px=480;
int x_offset=0,y_offset=400;
int max_fps=400;
...
int main(int argc, char **argv)
{
    ...
    nh.getParam("exp_tim",exp_tim);
    nh.getParam("img_weight",x_px);
    nh.getParam("img_height",y_px);
    nh.getParam("x_offset",x_offset);
    nh.getParam("y_offset",y_offset);
    nh.getParam("max_fps",max_fps);
    ...
    //
    g_pRgbBuffer = (unsigned char*)malloc(x_px*y_px*3);
    ...
    */
    ...
    CameraSetExposureTime(hCamera,  exp_tim*1000);//ms曝光
    		tSdkImageResolution * 	pImageCustom;
    CameraCustomizeResolution(hCamera,pImageCustom);
    SetCameraResolution(hCamera, 0, 400, x_px, y_px);
